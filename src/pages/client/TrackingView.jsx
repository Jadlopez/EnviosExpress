    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col justify-between">
      ...
      <header className="p-4 sm:p-6 border-b border-slate-800 flex justify-between items-center max-w-5xl mx-auto w-full sticky top-0 bg-slate-900/90 backdrop-blur z-10">
        ...
        <h1 className="text-sm sm:text-base font-bold flex items-center space-x-2">
          ...
          <span className="truncate max-w-[180px] sm:max-w-none">Envíos Express - Rastreo</span>
        </h1>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12 w-full flex-grow">
        <div className="text-center mb-8 sm:mb-10">
          <h2 className="text-2xl sm:text-3xl font-extrabold mb-2 sm:mb-3">Rastrea tu Encomienda</h2>
          <p className="text-slate-300 text-sm sm:text-base px-2">
            Ingresa el código de guía suministrado al momento de despachar tu paquete.
          </p>
        </div>
        ...
        {resultado && (
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 sm:p-6 shadow-xl space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-700 pb-4">
              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wider block font-semibold">Código de Guía</span>
                <span className="text-lg sm:text-xl font-bold text-blue-400">{resultado.codigoGuia || codigoGuia}</span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm sm:text-base">
              <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
                <span className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Destinatario</span>
                <span className="font-semibold text-white">{resultado.nombreDestinatario || 'No especificado'}</span>
              </div>
              <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800">
                <span className="text-xs text-slate-400 uppercase tracking-wider block mb-1">Dirección de Destino</span>
                <span className="font-medium text-slate-200">{resultado.direccionDestino || 'No especificada'}</span>
              </div>
            </div>
      ...
      <footer className="text-center py-6 text-slate-500 text-xs border-t border-slate-800 px-4">
        Envíos Express &copy; {new Date().getFullYear()} - Todos los derechos reservados.
      </footer>
    </div>
  );
}